...
public class EReader extends Thread {
    ...
    protected boolean processMsg(int msgId) throws IOException {
    	synchronized( LOCK) {
    		return processMsg_(msgId);
    	}
    }
    
    protected boolean processMsg_(int msgId) throws IOException {
        ...
        switch( msgId) {
            ...
            case ORDER_STATUS: {
                ...
                eWrapper().onOrderStatus( id, status, filled, remaining, avgFillPrice,
            ...
            case PORTFOLIO_VALUE: {
                ...
                eWrapper().onPositionUpdate(contract, position, marketPrice, marketValue,
            ...
            case OPEN_ORDER: {
                ...
                // read order fields
                Order order = new Order( contract, orderId);
                order.m_side = readStr();
                order.m_size = readInt();
                ...
                eWrapper().onOpenOrder( order.m_orderId, contract, order, orderState);
                break;
            }
            ...
            case SCANNER_DATA: {
                ...
                for (int ctr=0; ctr < numberOfElements; ctr++) {
                    int rank = readInt();
                    if (version >= 3) {
                    	details.m_contract.m_conId = readInt();
                    }
                    details.m_contract.m_symbol = readStr();
                    details.m_contract.m_secType = readStr();
                    details.m_contract.expiry( readStr() );
                    details.m_contract.m_strike = readDouble();
                    details.m_contract.m_right = readStr();
                    details.m_contract.m_exchange = readStr();
                    details.m_contract.m_currency = readStr();
                    details.m_contract.m_localSymbol = readStr();
                    details.m_marketName = readStr();
                    details.m_tradingClass = readStr();
                    ...
                    eWrapper().scannerData(tickerId, rank, details, distance,
            ...
            case CONTRACT_DATA: {
                ...
                contract.m_contract.m_symbol = readStr();
                contract.m_contract.m_secType = readStr();
                contract.m_contract.expiry( readStr() );
                contract.m_contract.m_strike = readDouble();
                contract.m_contract.m_right = readStr();
                contract.m_contract.m_exchange = readStr();
                contract.m_contract.m_currency = readStr();
                contract.m_contract.m_localSymbol = readStr();
                contract.m_marketName = readStr();
                contract.m_tradingClass = readStr();
                contract.m_contract.m_conId = readInt();
                contract.m_minTick = readDouble();
                contract.m_contract.m_multiplier = readStr();
            ...
            case EXECUTION_DATA: {
                ...
                eWrapper().onTrade( orderId, contract, exec);
                break;
            }
            case MARKET_DEPTH: {
                int version = readInt();
            ...
            case HISTORICAL_DATA: {
              ...
              for (int ctr = 0; ctr < itemCount; ctr++) {
        		...
        		Bar bar = new Bar( date, time, high, low, open, close, volume, WAP, barCount);                
                eWrapper().onHistoricalBar(reqId, bar);
              }
              // send end of dataset marker
              eWrapper().onFinishedHistoricalData( reqId);
              break;
            }
            ...
            case REAL_TIME_BARS: {
                ...
                double wap = readDouble();
                int count = readInt();
        		Bar bar = new Bar( time, high, low, open, close, volume, wap, count);
                eWrapper().onRealtimeBar(reqId, bar);
                break;
            }
